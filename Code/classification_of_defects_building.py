# -*- coding: utf-8 -*-
"""Classification of defects building.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1v7Gqto3k5OLk1hLSknA0LJnhNxNGMOeR
"""
...
# Define your paths
original_images_path = '/content/drive/MyDrive/Building Defects/Combined_Equally_Images'
...
# Define layers as a list of (layer_name, color)
layers = [
    ("Input\n(224x224x3)", "lightgray"),
    ("DenseNet121\n(Pretrained Backbone)", "lightblue"),
    ("Global Avg Pooling", "lightgreen"),
    ("Dense (1024, ReLU)", "orange"),
    ("Dropout (0.6)", "red"),
    ("Dense (Softmax)", "yellow")
]
...
for i, (layer_name, color) in enumerate(reversed(layers)):
    ax.add_patch(mpatches.FancyBboxPatch(
        (0.1, i), 0.8, 0.8, boxstyle="round,pad=0.2",
        edgecolor="black", facecolor=color
    ))
    ax.text(0.5, i + 0.4, layer_name, fontsize=10, ha="center", va="center", fontweight="bold")
...
# Save & Show the diagram
plt.tight_layout()
plt.savefig("densenet_architecture.png", dpi=200)
plt.show()
...
# Load the welding defect image (replace 'welding_defect.jpg' with your image path)
def get_welding_image():
    img = cv2.imread("welding_defect.jpg")  # Provide the path to your welding defect image
...
layers = [
    ("Input Image\n(Welding Defect)", "gray"),
    ("Conv Layer", "lightblue"),
    ("Dense Block 1", "lightgreen"),
    ("Transition Layer", "lightgray"),
    ("Dense Block 2", "lightgreen"),
    ("Transition Layer", "lightgray"),
    ("Dense Block 3", "lightgreen"),
    ("Global Pooling", "orange"),
    ("Fully Connected Layer", "red"),
    ("Softmax Output\n(Defect Category)", "yellow")
]
...
def get_welding_image():
    ...
    image_path = os.path.join(current_directory, "welding_defect.jpg")
...
plot_model(densenet_model, to_file='densenet_model_small.png', show_shapes=True, show_layer_names=True, dpi=100)
...
model_xception.save('xception_defect_classifier1.h5')
...
y_pred = np.argmax(nasmodel.predict(X_test), axis=1)
y_true = np.argmax(y_test, axis=1)  # Assuming y_test is one-hot encoded
